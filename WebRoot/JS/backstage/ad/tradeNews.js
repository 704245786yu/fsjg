...
//新增
function add(){
	$('#ff').attr('action','tradeNews/saveTradeNews');
	g_bsFormTableExtend.showAddPanel();
}

//修改
function modify(id){
	$.post('tradeNews/getContent',{'id':id},function(data){
		g_bsFormTableExtend.showModifyPanel(id, 'tradeNews/updateTradeNews');
		var ue = UE.getEditor('editor');
		ue.setContent(data);
	});
}
...
function del(index,id){
	...
	if(confirm("确定要清空数据吗？")){
		$.get('tradeNews/delete/'+id,function(result){
			if(result == 1){

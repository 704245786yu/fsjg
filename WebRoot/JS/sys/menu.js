...
//	class1="treegrid-1"
//	class2="treegrid-2 treegrid-parent-1"
...
function search(){
	...
	if(param != ''){
		for(var i=0; i<data.length; i++){
			var searchText = data[i].menuName;
...
function modify(id){
	...
	new BsFormTableExtend().showModifyForm(id, 'menu/update');
}

//删除
function del(index,id){
	var action = 'menu/delete/';
	var title = null;
	var content = null;
	var icon = null;
	if(!$('.treegrid-'+id).treegrid('isLeaf')){
		title = '注意'
		content = '该菜单下的所有子菜单会一并删除，是否继续？';
		icon = 'warning';
	}
